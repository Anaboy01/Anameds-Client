...
const FundCard = ({owner, title, description, target, deadline, amountCollected, image, handleClick }) => {
  ...
  return (
    <div className={styles.container} onClick={handleClick}>
      <div className={`${styles.box} ${styles.hoverEffect}`}>
      <img src={image} alt='fund' className={styles.image}/>
      <div className={styles.tagCon}>
      <img src={tagType} alt='tag' className="w-[17px] h-[17px] object-contain" />
      <p className=" mt-[2px] font-epilogue font-medium text-[12px] text-[#808191]">Hospital</p>
      </div>
      <div className={styles.texts}>
      <div className={styles.more}>
      <h3 className='className="font-epilogue font-semibold text-[16px] text-left leading-[26px] truncate text-white'>{title}</h3>
          <p className="mt-[5px] font-epilogue font-normal text-[#808191] text-left leading-[18px] truncate">{shortenText(description, 17)}</p>
      </div>
         <div className={`flex justify-between flex-wrap mt-[15px] ${styles.time}`}>
         ...
         <div className={`${styles.addresss}`}>
          <div className="w-[20px] h-[20px] rounded-full flex justify-center items-center bg-[#13131a]">
          <img src={thirdweb} alt='user' className="w-1/2 h-1/2 object-contain"/>
          </div>
          <p className="flex-1 font-epilogue font-normal text-[12px] text-[#808191] truncate">by <span className="text-[#b2b3bd]">{shortenText(owner, 17)}</span></p>
